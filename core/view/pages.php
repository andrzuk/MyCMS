...
class Pages_View
{
	...
	public function ShowForm($row, $required, $failed, $import)
	{
		$id = 0;
		$main_page = 0;
		$system_page = isset($_SESSION['form_fields']['system_page']) ? $_SESSION['form_fields']['system_page'] : 0;
		...
		foreach ($import as $i => $j)
		{
			if ($i == 'category')
			{
				foreach ($j as $key => $value)
				{
					...
					{
						if ($k == 'id') $cat_id = $v;
						if ($k == 'caption') $cat_caption = $v;						
					}
					if ($cat_id == $category_id)
						$main_option = Array('value' => $cat_id, 'caption' => $cat_caption .' (id='. $cat_id .')', 'selected' => 'selected');
					else
						$main_option = Array('value' => $cat_id, 'caption' => $cat_caption .' (id='. $cat_id .')');
		...
		$form_data = Array(
						Array('type' => 'hidden', 'id' => 'main_page', 'name' => 'main_page', 'value' => $main_page),
	...
	public function ShowList($list, $columns, $params)
	{
		...
		$list_image = 'img/32x32/page_copy.png';
